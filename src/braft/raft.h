...
namespace braft
{
    ...
    {
        PeerStatus()
            : valid(false), installing_snapshot(false), blocking(false), next_index(0), last_rpc_send_timestamp(0), flying_append_entries_size(0), readonly_index(0), consecutive_error_times(0)
        {
        }
    ...
    struct NodeStatus
    {
        ...
        NodeStatus()
            : state(STATE_END), readonly(false), term(0), committed_index(0), known_applied_index(0), pending_index(0), pending_queue_size(0), applying_index(0), first_index(0), last_index(-1), disk_index(0)
        {
        }
    ...
    inline NodeOptions::NodeOptions()
        : election_timeout_ms(1000), catchup_timeout_ms(0), max_clock_drift_ms(1000), snapshot_interval_s(3600), catchup_margin(1000), usercode_in_pthread(false), fsm(NULL), node_owns_fsm(false), log_storage(NULL), node_owns_log_storage(true), filter_before_copy_remote(false), snapshot_file_system_adaptor(NULL), snapshot_throttle(NULL), disable_cli(false)
    {
    }
