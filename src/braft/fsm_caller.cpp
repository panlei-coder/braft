...
namespace braft
{
    ...
    FSMCaller::FSMCaller()
        : _log_manager(NULL), _fsm(NULL), _closure_queue(NULL), _last_applied_index(0), _last_applied_term(0), _after_shutdown(NULL), _node(NULL), _cur_task(IDLE), _applying_index(0), _queue_started(false)
    {
    }
    ...
    void FSMCaller::do_snapshot_load(LoadSnapshotClosure *done)
    {
        ...
        if (last_applied_id > snapshot_id)
        {
                                             ...
                                             " last_applied_index=%" PRId64 " last_applied_term=%" PRId64 " snapshot_index=%" PRId64 " snapshot_term=%" PRId64,
    ...
    IteratorImpl::IteratorImpl(StateMachine *sm, LogManager *lm,
        ...
        : _sm(sm), _lm(lm), _closure(closure), _first_closure_index(first_closure_index), _cur_index(last_applied_index), _committed_index(committed_index), _cur_entry(NULL), _applying_index(applying_index)
    {
        next();
    }
